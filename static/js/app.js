/* ── State ─────────────────────────────────────────────── */
    ...
    (function initParticles() {
        ...
        function draw() {
            ...
            const dotColor = isLight ? 'rgba(59,130,246,0.35)' : 'rgba(249,115,22,0.35)';
            const lineColor = isLight ? 'rgba(59,130,246,' : 'rgba(249,115,22,';
            ...
            for (let i = 0; i < particles.length; i++) {
                ...
                for (let j = i + 1; j < particles.length; j++) {
                    ...
                    if (dist < CONNECT_DIST) {
                        ...
                        ctx.moveTo(p.x, p.y);
                        ctx.lineTo(q.x, q.y);
                        ctx.strokeStyle = lineColor + (1 - dist / CONNECT_DIST) * 0.15 + ')';
                        ctx.lineWidth = 0.6;
                        ctx.stroke();
                    }
                }
    ...
    function toggleTheme() {
        ...
        if (html.getAttribute('data-theme') === 'light') {
            ...
            localStorage.setItem('echovox-theme', 'dark');
        } else {
            html.setAttribute('data-theme', 'light');
            if (icon) { icon.className = 'fas fa-sun'; }
            localStorage.setItem('echovox-theme', 'light');
        }
    }
    // Restore saved theme
    (function() {
        const saved = localStorage.getItem('echovox-theme');
        if (saved === 'light') {
    ...
    function logout() {
        ...
        // Reset admin login button state
        const adminBtn = document.getElementById('adminLoginBtn');
        if (adminBtn) {
            adminBtn.disabled = false;
            adminBtn.innerHTML = '<i class="fas fa-shield-alt"></i> Admin Sign In';
        }
            ...
            function connectSSE(taskId) {
                ...
                evtSource.onmessage = function(event) {
                    ...
                    if (taskData.error) {
                        ...
                        // Reset retry counter on successful messages
                        sseRetries = 0;
                ...
                evtSource.onerror = function(err) {
                    if (sseFinished) return; // already done, ignore disconnect
                    evtSource.close();
                    sseRetries++;
                    if (sseRetries <= 5) {
                        // Retry after a short delay — server may still be processing
                        console.warn(`SSE disconnected, retry ${sseRetries}/5 in 2s…`);
                        setTimeout(() => connectSSE(taskId), 2000);
                    } else {
                        if(progressInterval) clearInterval(progressInterval);
                        console.warn('SSE Error (gave up):', err);
                        showResultError(resultId, 'Lost connection to backend process. The server may have run out of memory loading the voice model.');
                        loading.classList.remove('show');
                        btn.disabled = false;
                    }
                };
    ...
    async function loadAdminDashboard() {
        ...
        // 4. Load new analytics sections concurrently
        fetch('/api/admin/usage-analytics').then(r => r.json()).then(renderUsageChart).catch(e => console.error('Usage load error:', e));
        fetch('/api/admin/activity-heatmap').then(r => r.json()).then(renderHeatmap).catch(e => console.error('Heatmap load error:', e));
        fetch('/api/admin/system-health').then(r => r.json()).then(renderSystemHealth).catch(e => console.error('System Health load error:', e));
    ...
    async function loadUserDirectory() {
        ...
        try {
            const res = await fetch(`/api/admin/users?search=${encodeURIComponent(search)}&filter=${filter}`, { cache: 'no-store' });
            ...
            tbody.innerHTML = users.map(u => `
                <tr style="border-bottom:1px solid rgba(255,255,255,0.04);">
                    ...
                    <td style="padding:0.8rem 1.25rem;">
                        <button onclick="toggleBanUser(${u.id}, this)" class="header-btn" style="padding:0.3rem 0.75rem; font-size:0.75rem;
    ...
    async function toggleBanUser(userId, btn) {
